...
// -------------------------------------------------------------------
void vectorcopy_simd( vfloat32 *vX1, vfloat32 *vX2, size_t size)
// -------------------------------------------------------------------
{
    vfloat32 x1;
...
vfloat32 vect_left1(vfloat32 v0, vfloat32 v1) // a b c d - e f g h -> b c d e
{    
    vfloat32 r; 
    //Completez le code ici
...
vfloat32 vect_right1(vfloat32 v0, vfloat32 v1) // a b c d - e f g h -> d e f g
{       vfloat32 r; 
	 //Completez le code ici
	return r;
}
...
int main()
{
	...
	// test pour la copie de vectors
	{
		auto start = std::chrono::steady_clock::now();
		vectorcopy_simd(vX1,vX2,array_size);
		auto end = std::chrono::steady_clock::now();
		std::chrono::duration<double> diff = end-start;
		// std::cout << "vector copy time with simd: " << diff.count() << " s" << std::endl;
		...
		{
			float out[4] ;
			_mm_store_ps(out , vX2[i]);

			if ( out[0] == vec1[j] && out[1] == vec1[j+1] && out[2] == vec1[j+2] && out[3] == vec1[j+3])
			{ j += 4;}
			else
			{
				is_valid = false;
		...
		if(is_valid) {std::cout << "la copie de vecteurs en simd est correcte" << std::endl;
					  std::cout << "speedup obtained for vector addition with simd : " << diff1.count() / diff.count() << std::endl;
					 }
		else {std::cout << " la copie de vecteurs en simd est incorrecte" << std::endl;}
